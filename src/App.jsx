...
const EV_DEF = {
  goal:      { l: "ゴール！",         i: "⚽", color: null },
  concede:   { l: "失点",             i: "🥅", color: null },
  nice:      { l: "ナイスプレー",     i: "✨", color: null },
  sub:       { l: "選手交代",         i: "🔄", color: null },
  close:     { l: "惜しい！",         i: "😤", color: null },
  nicepass:  { l: "ナイスパス！",     i: "👟", color: null },
  nicedef:   { l: "ナイスディフェンス！", i: "🛡️", color: null },
  nicegk:    { l: "ナイスキーパー！", i: "🧤", color: null },
};
...
function drawNewspaper(canvas, data, matchInfo) {
  ...
  ctx.fillStyle = NAVY;
  ctx.font = `bold ${W*0.09}px serif`; ctx.textAlign = "center";
  ...
  ctx.fillStyle = "#fff"; ctx.font = `bold ${W*0.1}px sans-serif`;
  ctx.fillText(":", W*0.5, sbY+sbH*0.82);
  const resultColor = data.result==="勝利"?"#27ae60":data.result==="敗戦"?"#c0392b":"#7f8c8d";
  const badgeY = sbY+sbH+W*0.03, badgeW = W*0.22, badgeH = W*0.06;
  ctx.fillStyle = resultColor; ctx.fillRect(W/2-badgeW/2, badgeY, badgeW, badgeH);
  ...
  ctx.fillStyle = NAVY; ctx.fillRect(W*0.05, divY, W*0.9, 2);
  ctx.fillStyle = NAVY; ctx.textAlign = "center";
  const hlSize = W*0.072; ctx.font = `bold ${hlSize}px serif`;
  const headline = data.headline||"FC DAURAT 勝利！";
  const maxW = W*0.88; let line = "", lines = [];
  for (let ch of headline.split("")) { const t=line+ch; if(ctx.measureText(t).width>maxW&&line){lines.push(line);line=ch;}else{line=t;} }
  if(line) lines.push(line);
  lines.forEach((l,i)=>ctx.fillText(l, W/2, divY+W*0.1+i*(hlSize*1.2)));
  const hlEndY = divY+W*0.1+lines.length*(hlSize*1.2);
  ctx.fillStyle = "#333"; ctx.font = `${W*0.042}px serif`;
  const sub = data.subheadline||""; let subLine="",subLines=[];
  for(let ch of sub.split("")){const t=subLine+ch;if(ctx.measureText(t).width>maxW&&subLine){subLines.push(subLine);subLine=ch;}else{subLine=t;}}
  if(subLine) subLines.push(subLine);
  subLines.forEach((l,i)=>ctx.fillText(l, W/2, hlEndY+W*0.02+i*(W*0.05)));
  const subEndY = hlEndY+W*0.02+subLines.length*(W*0.05);
  ctx.fillStyle = GOLD; ctx.fillRect(W*0.2, subEndY+W*0.02, W*0.6, 2);
  ctx.fillStyle = "#555"; ctx.font = `italic ${W*0.038}px serif`;
  const comment = `"${data.comment||""}"`;
  let cmLine="",cmLines=[];
  for(let ch of comment.split("")){const t=cmLine+ch;if(ctx.measureText(t).width>maxW*0.8&&cmLine){cmLines.push(cmLine);cmLine=ch;}else{cmLine=t;}}
  if(cmLine) cmLines.push(cmLine);
  cmLines.forEach((l,i)=>ctx.fillText(l, W/2, subEndY+W*0.07+i*(W*0.048)));
  const cmEndY = subEndY+W*0.07+cmLines.length*(W*0.048);
  ctx.font = `${W*0.06}px sans-serif`; ctx.fillStyle = GOLD;
  ctx.fillText("★".repeat(parseInt(data.rating)||5)+"☆".repeat(5-(parseInt(data.rating)||5)), W/2, cmEndY+W*0.07);
...
export default function App() {
  ...
  const saveToFirestore=async(m,aw)=>{
    try{await addDoc(collection(firestore,"matches"),{match:m,away:aw,sH:m.sH,sA:m.sA,matchType:m.matchType||"",tournamentName:m.tournamentName||"",savedAt:Date.now(),dateStr:nowStr()});}
    catch(e){console.error(e);}
  };

  const startNew=()=>{const nm=newMatch();setMatch(nm);setTimer("00:00");setPending("");syncToRTDB(nm,roster);setTab(0);};
  ...
  const commitEv=(type,team,num)=>{
    ...
    if(type==="goal"&&team==="home"){
      ...
      clearTimeout(flashRef.current);flashRef.current=setTimeout(()=>setFlash(false),3500);
    }
    setPending("");
  };
  ...
  const saveHistMatch=async(histIdx,newEvs)=>{
    ...
    const newMatch2={...h.match,evs:newEvs,sH,sA};
    try{
      await updateDoc(doc(firestore,"matches",h.id),{match:newMatch2,sH,sA});
      const updated=[...history];
      updated[histIdx]={...h,match:newMatch2,sH,sA};
      setHistory(updated);
      setEditingEv(null);
    }catch(e){console.error(e);}
  };
  ...
  const editHistEv=async(histIdx,evIdx,type,team,num,half,min)=>{
    ...
    const baseMin=half===2?Math.max(0,(min||1)-1-(h1ms/60000)):Math.max(0,(min||1)-1);
    const rms=Math.max(0,baseMin*60000);
  ...
  const addPlayer=()=>{
    ...
    const nr={...roster,[addNum]:addName};setRoster(nr);setAddNum("");setAddName("");syncToRTDB(match,nr);
  };
  ...
  const C={
    wrap:{width:"100%",maxWidth:480,margin:"0 auto",padding:"0 0 40px",fontFamily:"sans-serif",boxSizing:"border-box"},
    hd:{background:NAVY,color:"#fff",padding:"12px 14px",borderBottom:`3px solid ${GOLD}`,marginBottom:0},
    tab:(on)=>({flex:1,padding:"10px 4px",fontSize:11,fontWeight:500,border:"none",borderBottom:on?`2px solid ${GOLD}`:"2px solid transparent",background:on?NAVY:"#162b50",color:on?GOLD_L:"#8899bb",cursor:"pointer"}),
    btnP:(dis)=>({display:"block",width:"100%",padding:14,background:dis?"#eee":NAVY,color:dis?"#aaa":GOLD_L,border:"none",fontSize:16,fontWeight:500,cursor:dis?"default":"pointer",marginBottom:8,borderBottom:dis?"none":`2px solid ${GOLD}`,borderRadius:12}),
    btnS:(dis)=>({display:"block",width:"100%",padding:11,background:"#f5f5f5",color:dis?"#aaa":"#333",border:"0.5px solid #ccc",fontSize:14,fontWeight:500,cursor:dis?"default":"pointer",marginBottom:10,opacity:dis?.4:1,borderRadius:12}),
    ebtn:(cls,dis)=>{
      const b={borderRadius:10,border:"0.5px solid #ccc",padding:"14px 6px",fontSize:13,fontWeight:500,cursor:dis?"default":"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5,opacity:dis?.3:1};
    ...
    input:{width:"100%",padding:"10px 12px",fontSize:14,border:"0.5px solid #ccc",borderRadius:8,background:"#fff",color:"#333",boxSizing:"border-box"},
    sechd:{fontSize:12,fontWeight:500,color:"#888",margin:"10px 0 5px",paddingBottom:3,borderBottom:"0.5px solid #ddd"},
  };

  const tlStyle=(cls,isGoalHome)=>{
    const b={display:"flex",alignItems:"baseline",gap:6,padding:"7px 10px",border:"0.5px solid #ddd",background:"#fff",marginBottom:4,fontSize:13,lineHeight:1.4,borderRadius:4};
    ...
    return(
      <>
        {srt.map((e,i)=>{
          ...
          return(
            <div key={i} style={{...tlStyle(cls,isGoalHome),position:"relative"}}>
              ...
              <span style={{flex:1,fontSize:e.sys?11:13,color:e.sys?"#aaa":isGoalHome?"#7a4a00":"#333",fontWeight:isGoalHome?700:400}}>
                ...
                {isGoalHome&&<span style={{marginLeft:6,fontSize:12}}>🌟</span>}
              </span>
              {editable&&!e.sys&&(
                <div style={{display:"flex",gap:4,flexShrink:0}}>
                  <button onClick={()=>{
                    ...
                    setEditEvNum(e.num||"");
                    const h1ms=m.h1ms||0;
                    const off=e.half===2?h1ms:0;
                    const totalMs=e.rms+off;
                    setEditEvHalf(e.half||1);
                    setEditEvMin(Math.floor(totalMs/60000)+1);
  ...
  return(
    <div style={C.wrap}>
      ...
      <div style={C.hd}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            {match.matchType&&<span style={{fontSize:11,color:GOLD_L,opacity:.7}}>{match.matchType}</span>}
      ...
      <div style={{display:"flex",background:"#162b50",marginBottom:12}}>
        {TABS.map((t,i)=><button key={i} style={C.tab(tab===i)} onClick={()=>setTab(i)}>{t}</button>)}
      </div>
            ...
            <div style={{background:"#f8f8f8",borderRadius:10,padding:"12px 14px",marginBottom:12,borderLeft:`3px solid ${GOLD}`}}>
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {MATCH_TYPES.map(t=>(
                    <button key={t.value}
                      onClick={()=>setMatch(m=>({...m,matchType:t.value}))}
                      style={{
              ...
              <div style={{marginBottom:10}}>
                ...
                <input type="text" value={match.tournamentName||""} onChange={e=>setMatch(m=>({...m,tournamentName:e.target.value}))}
                  placeholder="例: 〇〇カップ、第3節 など" style={C.input}/>
              </div>
              <div>
                <label style={{fontSize:12,color:"#666",display:"block",marginBottom:4}}>相手チーム名</label>
                <input type="text" value={match.away||""} onChange={e=>setMatch(m=>({...m,away:e.target.value}))} placeholder="例: 光が丘FC" style={C.input}/>
              </div>
            </div>
          ):(
            <div style={{background:"#f8f8f8",borderRadius:10,padding:"8px 14px",marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:13,fontWeight:500,color:MATCH_TYPES.find(t=>t.value===match.matchType)?.official?"#e74c3c":NAVY}}>
                  {match.matchType}
                </span>
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:6,marginBottom:8}}>
                {[
                  ...
                  ["close","x","😤","惜しい！"],
                  ["nicepass","y","👟","ナイスパス！"],
                  ["nicedef","x","🛡️","ナイスディフェンス！"],
                  ["nicegk","y","🧤","ナイスキーパー！"],
                ].map(([type,cls,ico,lbl])=>(
                  <button key={type} style={{...C.ebtn(cls,!btnsOn),padding:"10px 6px",fontSize:12}} disabled={!btnsOn} onClick={()=>openDetail(type)}>
                  ...
                  <>
                    ...
                    <div style={{display:"flex",gap:8,marginBottom:8}}>
                      <div style={{flex:1}}>
                        ...
                        <select value={editEvHalf} onChange={e=>setEditEvHalf(Number(e.target.value))}
                          style={{width:"100%",padding:"8px 10px",fontSize:13,border:"0.5px solid #ccc",borderRadius:8,background:"#fff"}}>
                    ...
                    <div style={{display:"flex",gap:8,marginBottom:10}}>
                      <div style={{flex:1}}>
                        <label style={{fontSize:12,color:"#666",display:"block",marginBottom:4}}>チーム</label>
                        <select value={editEvTeam} onChange={e=>setEditEvTeam(e.target.value)}
                          style={{width:"100%",padding:"8px 10px",fontSize:13,border:"0.5px solid #ccc",borderRadius:8,background:"#fff"}}>
                          <option value="home">FC Daurat</option>
          ...
          {histDetail!==null?(
            <div>
              ...
              <div style={{background:NAVY,padding:"12px 14px",marginBottom:10,borderBottom:`2px solid ${GOLD}`,borderRadius:12}}>
                ...
                <div style={{textAlign:"center",fontSize:11,color:GOLD_L,opacity:.5,marginTop:4}}>
                  <span style={{color:MATCH_TYPES.find(t=>t.value===history[histDetail].matchType)?.official?"#ff9999":GOLD_L}}>
                    {history[histDetail].matchType||""}
                  </span>
                  {history[histDetail].tournamentName&&` · ${history[histDetail].tournamentName}`}
                  {" · "}{history[histDetail].dateStr}
                </div>
              </div>
              ...
              {histLoading?(
                ...
                history.map((h,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <button onClick={()=>{setHistDetail(i);setEditingEv(null);}} style={{flex:1,textAlign:"left",background:"#fff",border:"0.5px solid #ddd",borderRadius:12,padding:"12px 14px",cursor:"pointer"}}>
